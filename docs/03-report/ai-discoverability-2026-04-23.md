...
**범위**: 3 surface (GitHub 리포 `tomo-kay/tene` · 랜딩 `tene.sh` = `apps/web/` · CLI `cmd/ & internal/ & pkg/`)
...
- **Custom OG 이미지 미사용**: HN · X · Reddit 어디에 링크를 붙여도 `opengraph.githubassets.com/{hash}/tomo-kay/tene` 기본 자동 생성 이미지가 나옴. 브랜딩/인지 단계 누수.
...
| **설치 (curl·brew·docker·go install)** | ⚠️ README install 섹션 양호 · ⚠️ Homebrew 명령 예시가 없어 AI는 curl만 추천 | ⚠️ 설치 후 "다음 단계" 링크 약함 (퀵스타트/docs URL 히어로 부재) | 🔴 **`brew install tene` 작동 안 함** (.goreleaser.yml에 brews 섹션 자체 부재) · 🔴 **`docker run ghcr.io/tomo-kay/tene` 작동 안 함** · ⚠️ GPG 서명 미지원 (엔터프라이즈 이탈) |
...
- **현황**: `gh repo view` → `usesCustomOpenGraphImage: false`. HN·X·Reddit·Slack 어디에 `github.com/tomo-kay/tene` 를 붙여도 GitHub 자동 생성 이미지(파란 배경+저장소명)가 나옴.
...
- **개선**: `@graph` 배열에 2 노드 추가.
  ```ts
  {
    ...
    sameAs: ["https://github.com/tomo-kay/tene"],
    contactPoint: { "@type": "ContactPoint", contactType: "Support", url: "https://github.com/tomo-kay/tene/issues" },
  },
  {
    "@type": "WebSite",
...
- **현황**: `.goreleaser.yml` `dockers:` 섹션 없음. `ghcr.io/tomo-kay/tene` 이미지 미발행.
- **개선**:
  1. 루트에 `Dockerfile`:
     ```dockerfile
     FROM alpine:3.19
  ...
  2. `.goreleaser.yml`:
     ...
     ```
- **임팩트**: AI 가 CI/CD 맥락에서 추천하는 `docker run ghcr.io/tomo-kay/tene` 작동.
- **액션**: 45 분.
...
- **개선**: 2 줄 추가로 LLM-readable resource 경로 안내.
  ...
  info "  Docs:      https://github.com/tomo-kay/tene#readme"
  ```
...
- **개선**:
  ...
  Resources:
    Docs:      https://github.com/tomo-kay/tene
...
| 8 | `docker run ghcr.io/tomo-kay/tene version` | 실제 실행 | 200 OK (M2) |
...
gh api repos/tomo-kay/tene/traffic/views
gh api repos/tomo-kay/tene/traffic/popular/referrers
