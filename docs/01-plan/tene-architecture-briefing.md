...
> v3.1 (2026-04-06) — Local-Only MVP + AI Agent 자동 인식 + 12단어 니모닉 + @tene/cli 반영
>
...
**로컬에서 무료(서버 비용 $0)로 시크릿을 암호화 관리하는 CLI. AI Agent(Claude Code, Cursor)가 자동으로 tene 사용법을 인식하는 것이 핵심 차별점.**
...
┌────────────────────────────────────────────────┐
│                                                  │
│  tene CLI ─── @tene/crypto ─── OS Keychain       │
│       │                                          │
│       ├──── SQLite (.tene/vault.db)              │
│       │      암호화된 시크릿 저장                   │
│       │                                          │
│       └──── AI Agent 자동 인식                    │
│              CLAUDE.md / .cursorrules 자동 생성    │
│                                                  │
...
- **AI Agent 자동 인식**: `tene init` 시 CLAUDE.md/.cursorrules 자동 생성. AI가 즉시 tene을 인식
...
**뭘 하는가:** 시크릿의 암호화/복호화/저장/주입 + AI Agent 컨텍스트 자동 생성.

**기술:** TypeScript + Commander.js + better-sqlite3 + libsodium

| 명령어 | 하는 일 | 서버 필요 |
|--------|---------|:---------:|
| `tene init` | Master Password 설정, SQLite 볼트 생성, Recovery Key 발급, **CLAUDE.md 자동 생성** | X |
| `tene init --cursor` | 위 + **.cursorrules에 tene 가이드 추가** | X |
| `tene init --windsurf` | 위 + **.windsurfrules에 tene 가이드 추가** | X |
| `tene set KEY VALUE` | XChaCha20-Poly1305 암호화 → SQLite 저장 | X |
| `tene get KEY` | SQLite에서 읽기 → 복호화 → stdout 출력 | X |
| `tene run -- cursor .` | 모든 시크릿을 환경변수로 주입하고 명령 실행 | X |
...
**AI 에이전트 사용법:**
```bash
# AI 에이전트(Claude Code, Cursor)가 Bash에서 호출
...
- `tene init`만 하면 AI Agent가 tene을 자동으로 인식 (CLAUDE.md)
- Master Password 없이는 누구도 시크릿을 볼 수 없음
- `npm install -g @tene/cli` 하나로 설치 끝, 회원가입 없음

---

### 3.2 AI Agent 자동 인식 (핵심 차별점)

**뭘 하는가:** `tene init` 시 AI Agent 컨텍스트 파일을 자동 생성하여, AI Agent가 별도 설정 없이 tene 명령어를 인식.
...
**지원 Agent:**
- `--claude` (기본값): CLAUDE.md 생성
- `--cursor`: .cursorrules에 추가
- `--windsurf`: .windsurfrules에 추가

**왜 핵심 차별점인가:**
- 기존 시크릿 매니저 중 AI Agent 컨텍스트를 자동 생성하는 도구가 없음
- 개발자가 AI Agent에게 "시크릿은 tene으로 관리해"라고 매번 말할 필요 없음
- `tene init` 한 번이면 AI Agent가 영구적으로 tene을 인식
...
**기술:** better-sqlite3 (Node.js 네이티브 SQLite 바인딩)
...
my-project/CLAUDE.md     ← AI Agent 컨텍스트 (tene init이 자동 생성)
my-project/.cursorrules  ← Cursor용 (tene init --cursor)

글로벌 설정:
~/.tene/
└── config.json     ← CLI 설정, Fake Door 측정 데이터
    # salt는 각 프로젝트의 vault.db vault_meta에 kdf_salt로 저장 (프로젝트별 독립)
```

**왜 SQLite인가:**
- .env 파일 대비: 환경별 분리, 인덱스 검색, 부분 업데이트 가능
- 외부 DB 대비: 설치 불필요, 서버 불필요, 단일 파일
- better-sqlite3: 동기 API로 CLI 응답 < 10ms

---

### 3.4 @tene/crypto (암호화 코어 — 오픈소스)

**뭘 하는가:** 모든 암호화/복호화 로직.

**기술:** libsodium-wrappers
...
> 아래 구성요소는 Phase 2에서 구현한다. Lambda/Aurora Serverless를 사용하지 않고, ECS Fargate + RDS PostgreSQL을 사용한다.
...
**왜 ECS Fargate + NLB인가 (Lambda 대신):**
- Cold Start 없음 (Lambda: 100-500ms)
- 비용 예측 가능 (Fargate: 고정 과금)
- 운영 단순 (단일 컨테이너)
- RDS와 동일 VPC 내 통신
...
**왜 RDS PostgreSQL인가 (Aurora Serverless 대신):**
- 최소 비용 ~$15/월 (Aurora: ~$43/월)
- 예측 가능한 비용
- 충분한 성능 (db.t4g.micro로 시작)
...
서버 없음. AWS 비용 없음. npm 패키지 배포 비용만 (무료).
...
### Phase 1: MVP 로컬 CLI (2주) — 이것만으로 제품

| 주차 | 구현 대상 |
|------|----------|
| 1주 | @tene/crypto (Argon2id + XChaCha20 + Recovery Key) + SQLite 볼트 + init/set/get + **AI Agent 컨텍스트 생성** |
| 2주 | run/list/delete/import/export/env + export --encrypted + **Fake Door(tene sync)** + Keychain + npm 배포 |

**이 시점에서 제품 출시 가능.** 서버 없이, 비용 $0로, AI Agent가 자동 인식하는 시크릿 관리가 된다.

### Phase 2: $1 Cloud (수요 검증 후, 3-4주)

| 주차 | 구현 대상 |
|------|----------|
| 3주 | AWS 인프라 (ECS Fargate + NLB + RDS PostgreSQL + S3) + GitHub OAuth |
| 4주 | 동기화 엔진 (push/pull) + Stripe 결제 |
| 5주 | 웹 대시보드 (Next.js static) + 배포 |
...
tene/
├── packages/
│   ├── cli/          # tene CLI (npm global package)
│   ├── crypto/       # 암호화 코어 (CLI에서 사용, Phase 2 API에서도 재사용)
│   └── types/        # 공유 TypeScript 타입
├── docs/             # PM/Plan/Design 문서
└── (Phase 2에서 추가)
    ├── apps/
    │   ├── api/      # Cloud API (ECS Fargate + Hono)
    │   └── web/      # 웹 대시보드 (Next.js static)
    └── infra/
...
```
"시크릿의 Git" + AI Agent 자동 인식

MVP (Phase 1):                          Phase 2 (수요 검증 후):
- 로컬에서 무료로 동작 ($0)              - $1/월로 클라우드 백업
- 서버 불필요                            - 멀티 디바이스 동기화
- 오프라인 OK                            - 웹 대시보드
- AI Agent 자동 인식 (핵심!)             - 감사 로그
- Fake Door로 수요 검증

만드는 것 (MVP):
[CLI]         로컬 시크릿 암호화/관리          ← 핵심. 이것만으로 제품.
[SQLite]      암호화된 시크릿 로컬 저장         ← CLI의 일부.
[CLAUDE.md]   AI Agent 자동 인식               ← 핵심 차별점.
