# AI 에이전트 시크릿 관리 트렌드 v3.1
## AI Agent 자동 인식 트렌드 + Local-Only 시장 기회

> v3.1 (2026-04-06) — Local-Only MVP + AI Agent 자동 인식 반영
> 목적: AI Agent 자동 인식 트렌드 분석, CLAUDE.md/.cursorrules 생태계 기회
...
### 1.3 시크릿 누출과 AI Agent 자동 인식의 연결
...
- `tene init`이 이 빈 자리를 차지하면 **최초의 "AI 자동 인식 시크릿 관리 도구"**
- 선점 효과: 사용자가 CLAUDE.md에서 tene을 처음 보면, 다른 도구로 바꿀 이유가 없음

### 2.2 .cursorrules 생태계

**현황 (2026년):**
- Cursor AI가 프로젝트의 .cursorrules를 읽고 코딩 규칙 파악
- 커뮤니티에서 .cursorrules 공유 문화 형성 중
- AI 에디터의 "프로젝트별 규칙" 표준으로 확산

**Tene의 기회:**
- `tene init --cursor`로 .cursorrules에 시크릿 관리 가이드 자동 추가
- Cursor 사용자를 위한 별도 진입점
...
| **범용성** | Claude Code 전용 | Claude Code 전용 (Cursor는 .cursorrules) |
| **진입 장벽** | 높음 | **없음** |

---

## 3. NHI (Non-Human Identity) 시장 최신 동향
...
## 4. 경쟁사 AI 에이전트 대응 현황 (2026년 4월)

### 4.1 요약: 아무도 "자동 인식"을 안 한다
...
## 5. OWASP Top 10 for Agentic AI와 Tene
...
4. **Local-First 트렌드**: 개발자 클라우드 피로감 증가
5. **GitHub Secrets 보완 공백**: CI/CD만 지원, 로컬 개발 미지원
...
*Tene v3 Brainstorming Document 04/05*
