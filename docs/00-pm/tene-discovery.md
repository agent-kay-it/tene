# Tene Discovery Analysis v3.1
## Local-Only MVP + AI Agent 자동 인식: 기회 발견과 검증
...
v2에서는 "서버가 없다"를 핵심으로 잡았다. v3에서는 한 단계 더 나아간다:

> **"AI 에이전트가 시크릿 관리 도구를 자동으로 인식하면 어떨까?"**

현재 모든 시크릿 관리 도구 — Vault, Doppler, Infisical, 1Password — 에는 이 기능이 없다. AI 에이전트(Claude Code, Cursor)가 프로젝트를 열 때 시크릿 관리 도구의 존재를 자동으로 인식하고 사용법을 아는 것. 이것이 Tene v3의 진짜 차별점이다.

**핵심 메커니즘:**
- `tene init` → CLAUDE.md 자동 생성 (Claude Code가 프로젝트 열 때 자동 읽음)
- `tene init --cursor` → .cursorrules에 tene 가이드 추가 (Cursor가 자동 읽음)
...
| P9 | 프로젝트 간 시크릿 재사용이 불편 | Medium | 중간 | 글로벌 키 공유 (향후 검토) |
| P10 | 시크릿 관리 도구 자체가 해킹될 수 있다는 불안 | High | 높음 | **서버가 없으면 해킹 대상 없음** |

### 1.4 P3이 새로운 핵심 Pain Point인 이유
...
## 2. Assumptions: 핵심 가정 식별 (v3)

| # | 가정 (Assumption) | 유형 | v2 대비 변경 |
|---|------------------|------|-------------|
| A1 | 바이브코더의 75%+가 시크릿 관리에 불편을 느낀다 | Desirability | 유지 |
| A2 | **AI Agent 자동 인식(CLAUDE.md)이 핵심 차별점이다** | Desirability | **신규**: v3 핵심 가정 |
| A3 | 로컬 전용 + 서버 없음이 바이브코더에게 매력적이다 | Desirability | 유지 |
| A4 | **Cloud는 MVP에 불필요하다 (수요 검증 후 구축)** | Viability | **변경**: v2 Phase 1.1 → v3 Phase 2 |
| A5 | `tene sync` Fake Door로 Cloud 수요를 확인할 수 있다 | Viability | **신규** |
...
| A10 | 팀 기능 수요가 실제로 존재한다 (가설, 검증 필요) | Viability | 유지 |
...
| 순위 | 가정 | 검증 이유 |
|------|------|----------|
| 1 | **A2: AI Agent 자동 인식이 차별점인가** | v3 핵심 가정. CLAUDE.md 자동 생성이 실제로 사용자에게 가치를 주는지 |
| 2 | **A4: Cloud 없이 MVP가 성립하는가** | 수요 확인 전 Cloud 구축 비용을 피할 수 있는지 |
| 3 | **A8: "서버가 없다"가 차별점인가** | 마케팅 메시지 핵심. 개발자 반응 검증 |

---

## 4. Experiments: 검증 실험 설계 (v3)
...
### Experiment 4: 팀 기능 수요 검증 (A10) — Fake Door

| 항목 | 내용 |
|------|------|
| **방법** | Fake Door Test |
| **대상** | CLI 활성 사용자 |
| **가설** | 활성 사용자의 15%+가 "팀 시크릿 공유" 기능에 관심을 표시 |
| **메트릭** | "팀 기능 알림 신청" 클릭률 |
| **기간** | MVP 출시 후 8주 |
| **비용** | $0 |
| **성공 기준** | 관심 표시 15%+ |
...
## 5. Opportunity Solution Tree (OST) v3
...
```
   ...
   |  로컬 전용 시크릿 |       |  AI Agent        |       |  Cloud 확장       |
   |  관리의 극단적    |       |  자동 인식        |       |  (Phase 2, 수요   |
   |  간소화           |       |  (핵심 차별점)    |       |   검증 후)        |
   +--------+---------+       +---------+--------+       +----------+--------+
            |                           |                           |
   +--------+--------+       +---------+--------+       +----------+--------+
...
+--v--+          +---v--+ +--v--+          +---v--+ +---v--+          +---v--+
| S1  |          | S2   | | S3  |          | S4   | | S5   |          | S6   |
|로컬 |          |Master| |CLAUDE|         |.cursor| |tene  |          |Cloud |
|CLI  |          |Pass  | |.md   |         |rules | |sync  |          |백업  |
|+    |          |+     | |자동  |         |자동   | |Fake  |          |+동기 |
|SQLite|         |XCha20| |생성  |         |생성   | |Door  |          |화    |
|볼트  |         |Poly  | |      |         |      | |Test  |          |      |
+--+--+          +--+--+ +--+--+          +--+--+ +--+--+          +--+--+
   |                |       |                |       |                 |
+--v--------+  +---v----+ +--v--------+ +---v----+ +--v--------+ +---v----+
| E1: 로컬   | | E2:    | | E3: AI    | | E4:    | | E5: tene  | | E6:    |
| CLI 프로토 | | 마스터  | | 자동 인식 | | Cursor | | sync     | | 수요   |
| 타입 테스트 | | 패스워드| | 가치 테스 | | 통합   | | Fake Door| | 확인   |
|            | | UX 테스 | | 트        | | 테스트 | | waitlist | | 후     |
|            | | 트      | |           | |        | |          | | 결정   |
+------------+ +--------+ +-----------+ +--------+ +----------+ +--------+

[Phase 1: Free MVP]            [Phase 1 핵심]          [Phase 2: 가설]
CLI + 로컬 볼트                AI Agent 자동 인식      Cloud (수요 검증 필요)
```

### OST 상세 설명

#### Opportunity 1: 로컬 전용 시크릿 관리의 극단적 간소화 (MVP 코어)
> "서버 없이, 가입 없이, 3분 만에 시작"

- `npm install -g @tene/cli` → `tene init` → `tene set KEY VALUE` → `tene run -- cursor .`
- 서버 연결 불필요, 회원가입 불필요, 비용 $0
- SQLite로 로컬에 암호화된 시크릿 저장
- 오프라인 100% 동작
- `tene export --encrypted`로 수동 암호화 백업 제공

#### Opportunity 2: AI Agent 자동 인식 (v3 핵심 차별점)
> "AI 에이전트가 자동으로 인식하는 시크릿 관리 도구"

**Solution 3 (S3): CLAUDE.md 자동 생성**
...
### 8.3 Local-First 소프트웨어 트렌드와의 정합성
...
| 항목 | v2 (Local-Free + Cloud-Paid) | v3 (Local-Only MVP + AI Agent 자동 인식) |
|------|------------------------------|----------------------------------------|
| **코어 메시지** | "서버 없는 시크릿 관리. 무료. 오픈소스." | **"AI 에이전트가 자동으로 인식하는 시크릿 관리. 서버 없이. 무료."** |
| **핵심 차별점** | 서버 없음, $1 파괴적 가격 | **AI Agent 자동 인식 (CLAUDE.md/.cursorrules)** |
| **Cloud** | Phase 1.1 ($1/월) | **Phase 2 (수요 검증 후)** |
| **수요 검증** | Cloud 출시 후 전환율 측정 | **`tene sync` Fake Door + waitlist** |
| **백업** | $1 Cloud 암호화 백업 | **`tene export --encrypted` 수동 백업** |
| **정직한 범위** | 암묵적 | **만료 확인/로테이션 불가 명시** |
| **서버 인프라** | 미정 | **ECS Fargate + NLB + RDS PostgreSQL (서버리스 X)** |
