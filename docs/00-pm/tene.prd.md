# Tene PRD (Product Requirements Document) v3.1
## Local-Only MVP + AI Agent 자동 인식 시크릿 관리

> v3.1 (2026-04-06) — 암호화 알고리즘 XChaCha20-Poly1305 통일, 기술 스택 정합성 수정
> Version: 3.1
> Status: Draft
...
| **Solution** | Tene는 **AI 에이전트가 자동으로 인식하는** 시크릿 관리 CLI. `tene init` → CLAUDE.md 자동 생성으로 Claude Code가 즉시 인식. 로컬 XChaCha20-Poly1305 암호화. 서버 없음, 가입 없음, 비용 $0. |
| **Target User** | 솔로 바이브코더 (Claude Code, Cursor 사용자) |
| **Core Value** | "AI 에이전트가 자동 인식 + 서버 없음 = 해킹 대상 없음" |
| **MVP 범위** | CLI + SQLite + @tene/crypto. Cloud 없음, 서버 없음 |
...
| 원칙 | 설명 |
|------|------|
| **AI Agent 자동 인식** | `tene init` → CLAUDE.md 생성 → AI가 즉시 인식 |
| **Local-Only** | 서버 없이, 가입 없이, 로컬에서 완전히 동작 |
| **Zero Friction** | 설치부터 첫 시크릿 주입까지 3분 이내 |
| **Server-Free Security** | 서버가 없으면 해킹 대상도 없다 |
| **정직한 범위** | 못 하는 것(만료/로테이션)을 명확히 밝힌다 |

---

## 2. ICP (Ideal Customer Profile) — v3

### 2.1 Primary ICP: 솔로 바이브코더 (무료 사용자)

| 속성 | 상세 |
|------|------|
| **프로필** | AI 코딩 도구(Claude Code, Cursor)로 사이드 프로젝트 1-5개 진행 |
| **관리 시크릿** | 5-15개 |
| **현재 방법** | .env 파일 + .gitignore |
| **핵심 Pain** | "AI가 시크릿 도구를 모른다" + "서버에 시크릿 맡기기 불안" |
| **Trigger** | "tene init 하면 AI가 자동으로 인식한다고?" |
| **WTP** | **$0** (무료 로컬이면 충분) |
| **핵심 가치** | AI 자동 인식, 서버 없음, 가입 없음, 무료 |

### 2.2 Beachhead Segment

> **Beachhead: Claude Code/Cursor 사용 솔로 바이브코더**
> 이들이 무료로 Tene를 채택 → AI 자동 인식 경험 → 습관화 → Cloud 수요 발생 시 Phase 2

---

## 3. MVP 범위 정의 (v3)

### 3.1 MVP 원칙: CLI 로컬 전용, 서버 비용 $0

> **MVP = CLI + SQLite + @tene/crypto**
> **서버 없음, 회원가입 없음, 비용 없음**
...
| **P0** | `tene init --cursor` | .cursorrules에 tene 가이드 추가 | **MVP** |
...
| 구분 | 내용 |
|------|------|
| **해결** | 키값 안전 저장, XChaCha20-Poly1305 암호화, 프로젝트별 관리, 환경변수 주입, AI Agent 자동 인식 (CLAUDE.md/.cursorrules) |
| **부분적** | 프로젝트 간 시크릿 재사용 (글로벌 키 공유는 향후 검토) |
| **못 함** | API key 만료 기간 확인, 자동 갱신/로테이션 (Phase 2+ 가설), 클라우드 동기화 (Phase 2) |

### 3.4 MVP 핵심 플로우 (서버 없음, AI 자동 인식)

```
[사용자의 시크릿 관리 - 로컬 전용 + AI 자동 인식]

1. 설치 (가입 없음!)
   $ npm install -g @tene/cli

2. 프로젝트 초기화 + AI 자동 인식 설정
   $ cd my-project
   $ tene init
   ? Master Password 설정: ********
   ? Master Password 확인: ********
...
### 4.2 .cursorrules 자동 추가

`tene init --cursor` 실행 시 .cursorrules 파일에 다음 내용이 추가된다:
...
## 6. User Stories (v3 — MVP 전용)

### 6.1 Epic 1: 로컬 시크릿 관리 + AI 자동 인식 (MVP Core)

| ID | User Story | 우선순위 |
|----|-----------|:--------:|
| US-01 | 바이브코더로서, `tene init`으로 Master Password를 설정하고 로컬 볼트를 생성하며 **CLAUDE.md를 자동 생성**할 수 있다, AI 에이전트가 즉시 인식하므로. | **P0** |
| US-02 | 바이브코더로서, `tene init --cursor`로 .cursorrules에 가이드를 추가할 수 있다, Cursor AI가 즉시 인식하므로. | **P0** |
| US-03 | 바이브코더로서, `tene set KEY VALUE`로 시크릿을 로컬에 암호화 저장할 수 있다. | **P0** |
| US-04 | 바이브코더로서, `tene get KEY`로 시크릿을 조회할 수 있다, AI 에이전트가 Bash에서 사용하므로. | **P0** |
| US-05 | 바이브코더로서, `tene run -- cursor .`로 시크릿이 주입된 환경에서 에디터를 실행할 수 있다. | **P0** |
| US-06 | 바이브코더로서, `tene list`로 시크릿 목록을 볼 수 있다. | **P0** |
| US-07 | 바이브코더로서, `tene delete KEY`로 시크릿을 삭제할 수 있다. | **P0** |
| US-08 | 바이브코더로서, `tene import .env`로 기존 .env 파일을 마이그레이션할 수 있다. | **P1** |
| US-09 | 바이브코더로서, `tene export`로 .env 형식으로 내보낼 수 있다. | **P1** |
| US-10 | 바이브코더로서, `tene export --encrypted`로 암호화된 백업을 생성할 수 있다. | **P1** |
| US-11 | 바이브코더로서, `tene env dev/prod`로 환경을 전환할 수 있다. | **P1** |
| US-12 | 바이브코더로서, 오프라인에서도 모든 CLI 명령이 동작한다. | **P0** |
...
### 6.3 Epic 3: Cloud (Phase 2 — 수요 확인 후)

| ID | User Story | 상태 |
|----|-----------|:----:|
| US-14 | 사용자로서, 클라우드에 암호화 백업 + 멀티 디바이스 동기화할 수 있다. | **Phase 2** |
| US-15 | 사용자로서, 웹 대시보드에서 시크릿 현황을 볼 수 있다. | **Phase 2** |

### 6.4 Epic 4: 팀 기능 (Phase 2+ — 가설)

| ID | User Story | 상태 |
|----|-----------|:----:|
| US-16 | 팀 리더로서, 팀 볼트를 생성하고 팀원을 초대할 수 있다. | **가설** |
...
| 구성요소 | 기술 | 근거 |
|----------|------|------|
| **언어** | TypeScript (Node.js) | npm 배포, 바이브코더 생태계 |
| **CLI 프레임워크** | Commander.js | 경량, 검증됨 |
| **로컬 DB** | **better-sqlite3** | Node.js 최고 SQLite 바인딩 |
| **암호화** | **@tene/crypto** (XChaCha20-Poly1305, libsodium) | 자체 암호화 모듈 |
| **KDF** | **Argon2id** (libsodium 내장) | 현대적 키 유도, libsodium-wrappers에서 제공 |
| **배포** | npm | `npm install -g @tene/cli` |

### 7.2 Phase 2 Cloud 기술 스택 (수요 확인 후, 서버리스 사용 안 함)
...
| 항목 | 요구사항 | 측정 기준 |
|------|----------|----------|
| **성능** | CLI 명령 응답 < 200ms | P95 latency |
| **오프라인** | 100% 동작 | 네트워크 차단 테스트 |
| **보안** | Master Password + XChaCha20-Poly1305 + Argon2id | 코드 리뷰 + 보안 감사 |
| **AI 인식** | CLAUDE.md 자동 생성, .cursorrules 자동 추가 | 통합 테스트 |
| **용량** | 시크릿 1,000개 이상 지원 | SQLite 성능 테스트 |
| **호환성** | macOS, Linux, Windows (WSL) | CI 테스트 |

---

## 8. GTM (Go-To-Market) 전략 (v3)

### 8.1 핵심 GTM: 오픈소스 CLI + AI 자동 인식 → 커뮤니티 성장 → 수요 검증
...
npm install -g @tene/cli
   |
   v
"AI 에이전트가 자동 인식하는 시크릿 관리" 메시지
   |
   v
GitHub 오픈소스 → Stars → 개발자 발견
   |
   v
목표: npm 설치 10,000 / GitHub Stars 1,000

[Stage 2: 수요 검증 (Month 3-6)]
tene sync Fake Door → waitlist 가입 수 확인
   |
   v
npm 다운로드 추세, GitHub Stars 추세 분석
   |
   v
Cloud 구축 여부 결정
...
| Step | 방법 | 성공 기준 |
|------|------|----------|
| **Step 1** | CLI 출시 → npm 다운로드, GitHub Stars 추적 | npm 주간 500+, Stars 1,000+ |
...
| 고객 반론 | Tene 대응 |
|-----------|----------|
| ".env로 충분한데?" | "Tene도 로컬이고 무료입니다. 하지만 암호화되어 있고, **AI가 자동으로 인식**합니다. `tene import .env` 한 줄이면 전환 끝." |

#### vs GitHub Secrets
| 고객 반론 | Tene 대응 |
|-----------|----------|
| "GitHub Secrets 쓰면 되잖아?" | "GitHub Secrets는 CI/CD 파이프라인 전용입니다. 로컬 개발 중에는 사용할 수 없습니다. Tene는 로컬 개발 + AI 에이전트 전용입니다. 완전히 다른 제품이며 보완적입니다." |

#### vs Doppler
| 고객 반론 | Tene 대응 |
|-----------|----------|
| "Doppler 잘 쓰고 있는데?" | "Doppler는 서버에 시크릿을 보내야 하고 $21/유저/월입니다. Tene는 로컬 전용이고 무료입니다. AI가 자동 인식합니다." |

#### vs Infisical
| 고객 반론 | Tene 대응 |
|-----------|----------|
| "Infisical도 오픈소스인데?" | "Infisical은 셀프호스팅 서버가 필요합니다. Tene는 서버 자체가 불필요하고, AI가 자동 인식합니다." |

#### vs Dotenvx
| 고객 반론 | Tene 대응 |
|-----------|----------|
| "Dotenvx도 로컬이고 암호화하던데?" | "Dotenvx는 .env 파일을 암호화합니다. Tene는 SQLite 볼트 + AI 자동 인식(CLAUDE.md)을 제공합니다. AI가 Tene 사용법을 알아서 인식합니다." |

---

## 9. Pre-mortem 분석 (v3)

### 9.1 "Tene가 실패한 이유는..." (v3)

| # | 실패 시나리오 | 확률 | 심각도 | 대응 |
|---|-------------|:----:|:------:|------|
| **R1** | ".env로 충분": 전환 동기 부족 | 높음 | 높음 | AI 자동 인식이 .env에 없는 핵심 가치. `tene import .env` 전환 비용 제로 |
...
| **R4** | Dotenvx AS2와 직접 경쟁 | 중간 | 높음 | AI 자동 인식 + SQLite 볼트로 차별화 |
| **R5** | Cloud 수요 없음: waitlist 반응 미미 | 중간 | 중간 | CLI 무료 도구로서의 가치에 집중, 수익 모델 재검토 |
| **R6** | 만료/로테이션 불가에 대한 불만 | 낮음 | 중간 | 정직하게 "못 하는 것" 명시, Phase 2+ 가설로 관리 |
...
> **npm 주간 설치 수** (Weekly npm Installs)

### 10.2 단계별 KPI (v3)

| 지표 | MVP (3개월) | 수요 검증 (6개월) | Phase 2 (12개월) |
|------|:----------:|:----------------:|:---------------:|
| npm 주간 설치 | 500 | 2,000 | 5,000 |
...
### 10.3 MVP 출시 기준 (Definition of Done) — v3
...
- [ ] XChaCha20-Poly1305 + Argon2id 암호화 구현 (@tene/crypto)
- [ ] Recovery Key 생성 + 복구 플로우
- [ ] 오프라인 100% 동작 확인
- [ ] macOS + Linux 설치 가능 (npm)
- [ ] CLI 응답 시간 < 200ms (P95)
- [ ] 자동화 테스트 커버리지 > 80%
- [ ] GitHub MIT 오픈소스 공개
...
-------------------        -------------------        -------------------
Phase 1: Local MVP         수요 검증                   Phase 2: 수요 확인 시

[CLI 코어 - 무료, $0]      [검증]                     [Cloud 구축 (수요 시)]
- set/get/run/list         - npm 다운로드 추적         - ECS Fargate + NLB
...
- Recovery Key             
                           [기능 강화]                [시장 검증]
[AI Agent 자동 인식]        - CLI 안정화               - 팀 기능 Fake Door
- CLAUDE.md 자동 생성      - 사용자 피드백 반영        - 에이전트 스코핑 검토
- --cursor 지원            - export --encrypted
                             개선
[출시]                     
- GitHub 오픈소스 (MIT)    [수요 검증]                [보안]
- npm 배포                 - tene sync Fake Door       - 보안 감사 (외부)
- Hacker News              - waitlist 가입 수 확인     - 버그 바운티 검토
- GeekNews                 
```
...
| 문서 | 경로 | 내용 |
|------|------|------|
| Discovery v3 | `docs/00-pm/tene-discovery.md` | AI Agent 자동 인식 기회, Fake Door Test |
| Strategy v3 | `docs/00-pm/tene-strategy.md` | AI 자동 인식 포지셔닝, $0 MVP 전략 |
| Market Research v3 | `docs/00-pm/tene-research.md` | AI Agent 통합 비교, GitHub Secrets 차별점 |

---

*Generated by PM Agent Team | 2026-04-06 (v3)*
